...
`scripts/reliability_cloud.py` runs in an authenticated OCI Cloud Shell task checkout. It packages only the Terraform configuration and provider lock. Each action is separate: `create-stack`, `plan`, `job`, `review`, `apply-reviewed`, `outputs`, `bootstrap`, `drift`. The first apply must match the exact 17-resource create-only allowlist and the previously reviewed plan digest. It cannot silently adopt existing resources. A failed/ambiguous create must be reconciled in the console before retrying, to avoid duplicates. Later updates require a new reviewed workflow; the initial-create guard intentionally rejects in-place updates/deletes.
...
4. Renew an approved Bastion session. Copy the non-secret-ID `config.json` privately to `/home/opc/llm-serving-observatory/.reliability/config.json`. Directory mode 0700 and file mode 0600, owned by UID 10001 for the worker. Create a protected runtime environment file with `RELIABILITY_PRIVATE_DIR` set to that directory and `OCI_APM_ENDPOINT` set to the provisioned upload origin. Never put the key itself in this file, shell history, Compose environment or Git.
...
6. Verify restore, metrics and instance-principal access from the **actual container**. Read-only SQLite WAL access and metadata-network routing are live gates, not assumed from unit tests. Install the provided systemd service/timers only after these pass. Units use the existing explicit checkout path and never invoke an unbounded full Compose stack.
