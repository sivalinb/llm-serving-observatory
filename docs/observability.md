...
- **Private native dashboard:** `/observability` reuses shared-profile Prometheus and SQLite. Personal keys reveal only owned records; separate operator grants allow bounded metric queries, all-project metadata and alert states. Sanitized application events are not raw container logs. See [access, architecture and limits](observability-dashboard.md).
...
The paragraphs below describe the **full isolated lab stack**, not the slim shared-host deployment. The shared profile has a native private dashboard, Prometheus with 24h / 128 MB block retention and three rules, but no Grafana, collector, Tempo or raw log backend. Model cgroup and host CPU/disk history are not exported there. [Shared-host boundaries](oci-shared-host.md#4-observability-with-a-small-footprint).
